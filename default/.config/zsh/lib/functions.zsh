...
brew_install() {
  /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)" && brew tap Homebrew/bundle && brew bundle install --file ~/.config/brew/Brewfile
}
...
update_hosts(){
  sudo curl -o /etc/hosts  https://raw.githubusercontent.com/StevenBlack/hosts/master/alternates/fakenews-gambling-porn/hosts && sudo dscacheutil -flushcache;sudo killall -HUP mDNSResponder
}